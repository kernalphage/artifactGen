import {isLiteral, isAlphaNumeric, isDigit, getWholeChar} from "./kp.js";
...
export class Scanner {
    ...
    isLiteraLCharacter(c){
        if( _.keys(singleTokens).includes(c) ) return false;
        if (_.keys(literals).includes(c)) return false;
        return true;
    }
    scanLiteral() {
...
/// helper functions for moving along the source 
    ...
    peekline(){
        let alreadyParsed = this.source.substring(0, this.current);
        let idx = this.findLastIndex(alreadyParsed, '\n');
        return this.source.substring(idx, this.current);
    }
    error(message) {
        throw new ScannerError("Error at " + this.line + ":" + this.current + ": " + message, this.peekline());
    }
    isAtEnd() {
    ...
    advance() {
        ...
        do{
            this.current++;
        } while(getWholeChar(this.source, this.current) === false);
        return c;
    }
