...
export class Parser {
    ...
    error(message){
        ...
        throw new ParserError(message + " around " + curLine, this.peek());
    }
    ...
    find_some(parseFn, sepToken, endToken, mustFind){
        ...
        if (!(parseFn instanceof Function)) {
            let sym = parseFn;
            parseFn = () => {
                return this.mustMatch(sym, "Could not find the right token while parsing list separated by " + sepToken.toString());
            };
        }
    ...
    parse_lvalue() {
        ...
        return Ex(pr.LValue, reftype, locator);
    }
    ...
    parse_rvalue() {
        ...
        return new Ex(pr.RValue, reftype, locator);
    }
