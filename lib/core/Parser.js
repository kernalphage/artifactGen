...
function Ex(t, ...args){
    console.log("Creating expression  of type " + t.toString());
    return _Ex(t, ...args);
}
...
export class Parser {
    ...
    mustMatch(tokens, errorMessage){
        ...
        return m ? m : this.error(errorMessage + "(Expected [" + _.map(tokens, (t) => t.toString()).join(", ") + "] found " + this.peek() + ")");
    }
    ...
    parse_main() {
        ...
        while (!this.isAtEnd()) {
            ...
            try{
                ...
                console.log("Parser error found: " + e.message);
                this.errors.push(e);
                this.synchronize();
            }
        }
