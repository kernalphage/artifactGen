...
const I_Interpreter_js_1 = require("./I_Interpreter.js");
...
;
class BasicInterpreter extends I_Interpreter_js_1.I_Interpreter {
    constructor() {
        super();
        this.contexts = [];
        this.visitors = {};
        this.env = new Environment_js_1.Environment(".");
        this.toResolve = [];
        this.visitors = {
            "Assignment": (expr, ctx) => {
                ...
                _.zip(expr.targets, chosenAssignment.statements).map((assignment) => {
                    ...
                    let targetLoc = this.visit(target);
            ...
            "Definition": (expr) => {
                let name = expr.id;
                let def = new Environment_js_1.Environment(name);
            ...
            "EXNumber": (expr) => {
                if (expr.numbers.length == 1) {
                    ...
                    return kp_js_1.randomRangeInt(expr.numbers[0], expr.numbers[1]);
                }
            },
            "RValue": (expr, ctx) => {
                try {
