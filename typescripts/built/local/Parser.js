...
class Parser {
    ...
    error(ErrorType, message) {
        ...
        throw new ErrorType(message + "at >" + this.peek() + "< Current line " + curLine);
    }
    parse_main() {
        while (!this.isAtEnd()) {
            try {
    ...
    parse_base_expr() {
        ...
        return new Expressions_js_1.Base(values);
    }
    ...
    parse_statement() {
        ...
        return new Expressions_js_1.Statement(values);
    }
    parse_number() {
    ...
    parse_assignment() {
        ...
        return new Expressions_js_1.Assignment(targets, values);
    }
    parse_locator() {
        let loc = this.find_some(14, 6);
        let locations = _.map(loc, (t) => t.value);
        return new Expressions_js_1.Locator(locations);
    }
    parse_definition() {
        ...
        let defExpr = new Expressions_js_1.Definition(name, assignments);
        return defExpr;
    }
    parse_lvalue() {
        ...
        return new Expressions_js_1.LValue(reftype, locator);
    }
    parse_rvalue() {
        ...
        return new Expressions_js_1.RValue(reftype, locator);
    }
    parse_function_expr() {
        ...
        return new Expressions_js_1.ExFunction(loc, params);
    }
}
